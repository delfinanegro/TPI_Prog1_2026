...
# Guarda el puntaje en un archivo (Adaptado con 'with' como el de tu compañera)
...
def ver_ranking():
    ...
    try:
        # Adaptado con 'with' y validación de datos para evitar errores de lectura
        with open("ranking.txt", "r") as archivo:
            ranking = {}

            for linea in archivo:
                datos = linea.strip().split(",")
                if len(datos) == 2:  # Asegura que la línea tenga usuario y puntaje
                    usuario = datos[0]
                    puntaje = int(datos[1])
